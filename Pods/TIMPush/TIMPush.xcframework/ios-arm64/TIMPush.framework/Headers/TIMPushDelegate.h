...
//
// 【二、注册推送服务】
// 1.当您通过 (pod 'TIMPush') 方式使用推送服务时:
// 您需要使用 [TUILogin](https://github.com/TencentCloud/TIMSDK/blob/master/iOS/TUIKit/TUICore/TUILogin.h) login/logout 接口登录/登出
// 此时 TIMPush 组件会自动感知登录/登出事件,帮您进行推送服务的注册/反注册。
//
// 2.当您通过 (pod 'TPush') 方式使用推送服务时:
// 仅需要在您启动 App 时调用 `+ (void)registerPush:userId:inAppKey:succ:fail:` 即可完成推送服务的注册。
//
// 至此，您已经完成了基本推送功能的接入，注意：
// - 当您完成以上操作后，在控制台上看到 APNs configuration success 日志打印时，即表示已成功接入；
// - 如果您的 APP 已经获取到了推送权限，此时退入后台或者杀死进程，即可收到远程推送通知。
//
// 【三、监听推送点击事件】
// 1. 如果您需要自定义解析收到的远程推送，您需要在 AppDelegate.m 文件中实现 `- onRemoteNotificationReceived` 方法;
// 2. [当您集成了 TUIChat 时]如果您想使用内置的 TUIChat 推送解析逻辑，并跳转到 TUIChat 的聊天页面，您可以实现 `- navigateToBuiltInChatViewController` 方法。
//
// 【四、统计推送抵达率】
